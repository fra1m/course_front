...
import AdminUserCreatePage from '../../components/Modals/AdminUserCreatePage';
...
// форма редактирования (+ specializationId)
type EditFormFields = {
	name: string;
	role: Role;
...
export default function UsersPage() {
	...
	const [editForm] = Form.useForm<EditFormFields>();
	...
	useEffect(() => {
		if (authReady && isAuth && specs.length === 0) {
			dispatch(fetchSpecializations());
		}
	}, [dispatch, specs.length, authReady, isAuth]);

	// ---- ФЕТЧ ПОЛЬЗОВАТЕЛЕЙ: один раз, после авторизации, и если ещё не загружено ----
	...
	const openEdit = useCallback(
		(u: IUserRow) => {
			...
			editForm.setFieldsValue({
				name: u.name ?? '',
				role: u.role,
				specializationId: u.specialization?.id ?? null,
			});
			setEditOpen(true);
		},
		[editForm]
	...
	const columns = useMemo<ColumnsType<IUserRow>>(() => {
		const cols: ColumnsType<IUserRow> = [
			...
			{
				title: 'Специализация',
				dataIndex: ['specialization', 'title'],
				key: 'specialization',
				width: 260,
				render: (_: unknown, r) => (
					<Space size={6}>
						<ApartmentOutlined />
						<Text style={{ color: subtleText }}>
							{r.specialization?.title ?? '—'}
						</Text>
					</Space>
				),
			},
			...
			<Col>
				...
				>
					<Form form={editForm} layout='vertical'>
						...
						{/* поле специальности (доступно для выбора; логически актуально для student) */}
						<Form.Item
							noStyle
							shouldUpdate={(prev, curr) => prev.role !== curr.role}
						>
							{() => (
								<Form.Item label='Специализация' name='specializationId'>
									<Select<number>
										allowClear
										loading={specsLoading}
										placeholder='Выберите специализацию'
										disabled={editForm.getFieldValue('role') !== 'student'}
										options={specOptions}
										style={{ width: '100%' }}
									/>
								</Form.Item>
