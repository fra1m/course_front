//FIXME нужно если теста нет, то урок пройден по кнопке к следующему уроку
...
import type { ISurveyJson } from '../../models/quiz/IServeyJSON';
import { submitQuizResult } from '../../store/reducers/analytics/analyticsThunks';
...
export const QuizPage: FC = () => {
	...
	const surveyJson: ISurveyJson = state.surveyJson;
	...
	const currentLesson = useMemo<ILesson | undefined>(() => {
		...
		return lessons.find((l: ILesson) => l.testId?.id === Number(quizId));
	}, [lessons, quizId]);
	...
	const survey = useMemo(() => {
		...
		m.showProgressBar =
			surveyJson.showProgressBar && surveyJson.showProgressBar !== 'none'
	...
	useEffect(() => {
		...
		const onComplete = async (s: Model) => {
			let correct = 0;
			let total = 0;
			...
			try {
				...
				message.warning(
					typeof e === 'string' ? e : 'Не удалось сохранить результат'
				);
			}
		};
	...
	}, [currentLesson, dispatch, message, quizId, survey]);
