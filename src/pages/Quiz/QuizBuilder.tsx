//TODO: добавить возможность как вопрос - изображения
//TODO: добавить возможность как в ответ на вопрос - текст
...
export const QuizBuilder = () => {
	...
	const validateQuiz = (): string[] => {
		...
		pages.forEach((page, pIdx) => {
			const q = page.elements?.[0]; // у тебя 1 вопрос на страницу
			const label = page.title || `Вопрос ${pIdx + 1}`;

			// 1) Заголовок вопроса обязателен (кроме html, который служебный)
			if (q?.type !== 'html') {
				const qt = (q?.title ?? '').toString().trim();
				if (!qt) errors.push(`«${label}»: заголовок вопроса обязателен`);
			}

			// Только для типов с вариантами
			if (q && ['radiogroup', 'checkbox', 'imagepicker'].includes(q.type)) {
				const choices = Array.isArray(q.choices) ? q.choices : [];
				if (choices.length < 2) {
					errors.push(`«${label}»: минимум 2 варианта ответа`);
				}

				const correct = q.correctAnswer;

				if (q.type === 'radiogroup' || q.type === 'imagepicker') {
					const correctSet =
						correct !== undefined &&
						correct !== null &&
						correct !== '' &&
						choices.some(c => (c?.value ?? c?.text) === correct);
					...
					if (arr.length === 0) {
						errors.push(`«${label}»: выберите хотя бы один правильный ответ`);
					} else {
						// убедимся, что все выбранные есть среди вариантов
						const ok = arr.every(v =>
							choices.some(c => (c?.value ?? c?.text) === v)
						);
						if (!ok)
							errors.push(`«${label}»: выбран неверный правильный ответ`);
					}
				}
			}
		});
	...
	// Вспомогалка для плавного вывода ошибок
	const showErrors = (errs: string[]) => {
		...
		Modal.error({
			title: 'Исправьте ошибки перед сохранением',
			content: (
	...
	const removeChoice = (pageId: string, questionId: string, index: number) => {
		...
		if (question.choices.length <= 2) {
			message.warning('Вопрос должен содержать минимум 2 варианта ответа');
			return;
		}
					...
					{state.isUpdate ? (
						<Popconfirm
							...
							onConfirm={async () => {
								...
								try {
									await dispatch(updateQuiz(state)).unwrap();
